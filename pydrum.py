...
class PyDrum:
    ...
    def create_bass_drum(length=common.WAV_FREQ // 5):
        data = [0] * length
        ratio = [1.00, 1.31, 1.60, 1.88]  # modes
        ...
        for i in range(length):
            ...
            for ratio_index in range(len(ratio)):
                sheta[ratio_index] += (
                    math.pi * 2 * freq * ratio[ratio_index] / common.WAV_FREQ
                )
                sheta[ratio_index] %= math.pi * 2
                level = 1 / ((ratio_index + 1) ** 3)
                data[i] += osc.sin(sheta[ratio_index]) * level * 0.5

            # ビーターの計算
            beater = 0
            if i < 441:
                data[i] += osc.noise() * 0.05
    ...
    def create_snare_drum():
        ...
        for i in range(length):
            ...
            data.append(
                (math.sin(shita) * 0.3 + beater) * common.ratio_from_gain(p * -60)
            )
    ...
    def create_popup():
        ...
        for i in range(length):
            r = i / length
            p = math.pow(r, 2)
            theta += (1 - r) * delta_from + r * delta_to
            theta %= math.pi * 2
            data[i] = osc.square(theta)
    ...
    def create_cymbal():
        ...
        for i in range(0, int(common.WAV_FREQ * duration)):
            ...
            data.append(osc.noise() * 0.5 * common.ratio_from_gain(r * -60))
